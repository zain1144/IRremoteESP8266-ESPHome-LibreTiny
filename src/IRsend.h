...
// Optional timing-capture hooks. They let another framework provide the
// physical transmitter while IRremoteESP8266 remains responsible for
// protocol encoding. When no callbacks are installed, IRsend behaves exactly
// as upstream.
typedef void (*irsend_capture_mark_cb_t)(uint32_t usec);
typedef void (*irsend_capture_space_cb_t)(uint32_t usec);
typedef void (*irsend_capture_frequency_cb_t)(uint32_t hz, uint8_t duty);
