...
namespace {
irsend_capture_mark_cb_t capture_mark_cb = nullptr;
irsend_capture_space_cb_t capture_space_cb = nullptr;
irsend_capture_frequency_cb_t capture_frequency_cb = nullptr;
}  // namespace

void irsend_set_capture_callbacks(
    irsend_capture_mark_cb_t mark_cb,
    irsend_capture_space_cb_t space_cb,
    irsend_capture_frequency_cb_t frequency_cb) {
  capture_mark_cb = mark_cb;
  capture_space_cb = space_cb;
  capture_frequency_cb = frequency_cb;
}

void irsend_clear_capture_callbacks() {
  capture_mark_cb = nullptr;
  capture_space_cb = nullptr;
  capture_frequency_cb = nullptr;
}

bool irsend_capture_active() {
  return capture_mark_cb != nullptr && capture_space_cb != nullptr;
}
...
void IRsend::begin() {
  if (irsend_capture_active()) return;
#ifndef UNIT_TEST
...
void IRsend::enableIROut(uint32_t freq, uint8_t duty) {
  ...
  if (freq < 1000)  // Were we given kHz? Supports the old call usage.
    freq *= 1000;
  if (capture_frequency_cb != nullptr)
    capture_frequency_cb(freq, _dutycycle);
#ifdef UNIT_TEST
  _freq_unittest = freq;
#endif  // UNIT_TEST
...
uint16_t IRsend::mark(uint16_t usec) {
  if (capture_mark_cb != nullptr) {
    capture_mark_cb(usec);
    return 1;
  }
...
void IRsend::space(uint32_t time) {
  if (capture_space_cb != nullptr) {
    if (time != 0) capture_space_cb(time);
    return;
  }
  ledOff();
  if (time == 0) return;
